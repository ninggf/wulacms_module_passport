...
class OauthApi extends API {
    ...
    public function loginPost(string $type, string $openid, string $device, string $cid, string $unionid = '', string $recCode = '', string $channel = '', array $meta = null) {
        ...
        if (is_array($checked)) {
            if (isset($checked['openid'])) {
                ...
                unset($checked['openid']);
            }
            if (isset($checked['unionid'])) {
                $unionid = $checked['unionid'];
                unset($checked['unionid']);
            }
            if ($checked) {
                $meta = array_merge($meta ? $meta : [], $checked);
            }
        }
        // 从第三方获取用户信息
        $meta = $oapp->getOauthData($meta ? $meta : []);
        if ($meta === false) {
            $this->error(408, '第三方数据获取失败');
        }
        $db  = App::db();
    ...
    public function bindPost($token, $type, $openid, $device, $cid, $unionid, $force = 0, $meta = null) {
        ...
        if (!$checked) {
            $this->error(407, '[' . $oauthApp->getName() . ']数据校验失败了');
        }
        $meta = is_array($meta) ? $meta : [];
        // 从第三方获取用户信息
        $meta = $oauthApp->getOauthData($meta);
        $db   = App::db();
        $rst  = $db->trans(function (DatabaseConnection $dbx) use ($type, $openid, $unionid, $device, $info, $meta, $force) {
            ...
            if ($meta) {
                $remark = date('Ymd H:i:s');
                foreach ($meta as $key => $value) {
                    $rtn = $dbx->cud("INSERT INTO {oauth_meta} (oauth_id,name,value,remark) VALUES(%d,%s,%s,%s)", $data['id'], $key, $value, $remark);
                    if (!$rtn) {
                        $dbx->cud('UPDATE {oauth_meta} SET value=%s,remark=%s WHERE oauth_id=%d AND name=%s', $value, $remark, $data['id'], $key);
                    }
                }
            }
            fire('passport\bindOauth', $type, $info['uid']);
     ...
     * @apiName 第三方授权URL
     *
     * @param string $type    (required,sample=alipay) 第三方类型
     * @param string $device  (required) 设备
     * @param string $channel 渠道
     *
     * @paramo  string url 授权url(目前仅支持alipay)
     *
     * @error   400=>未知设备
     * @error   402=>不允许注册
     * @error   403=>注册关闭
     ...
     *
     * @return array {
     *  "url":"apiname=com.alipay.account.auth&app_id=xxxxx&app_name=mc&auth_type"
     * }
     * @throws
     */
    public function get_auth_url(string $type, string $device, string $channel) {
